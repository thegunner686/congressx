...
const MiniState = ({ state }: Props) => {
  return (
    <Link to={routes.stateSelection()}>
      <button
        className={` w-16 h-8 relative flex items-center justify-center cursor-pointer shadow-lg hover:scale-95 transition-all`}
      >
        <img
          src={state.imageUrl}
          className=" w-full h-full object-cover absolute top-0 left-0 rounded"
        />
        <h1 className="relative font-anton uppercase text-white text-shadow shadow-gray-500">
          {state.abbreviation}
        </h1>
      </button>
    </Link>
